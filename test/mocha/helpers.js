...
api.createInbox = function(mockData, options = {addId: true}) {
  ...
  if(options.addId) {
    inbox.id = brLdnReceiver.createInboxId();
  }
  return inbox;
};

api.createRawInbox = function(mockData, options = {addId: true}) {
  const inbox = bedrock.util.clone(mockData.rawInbox);
  if(options.addId) {
    inbox.id = brLdnReceiver.createInboxId();
  }
  return inbox;
...
api.createMessage = function(mockData, options = {addId: true}) {
  const message = bedrock.util.clone(mockData.message);
  if(options.addId) {
    message.id = brLdnReceiver.createMessageId();
  }
  return message;
};
