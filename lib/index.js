...
bedrock.events.on('bedrock-express.configure.routes', app => {
  const routes = bedrock.config['ldn-receiver'].routes;
  ...
  app.post(
    ...
    (req, res, next) => async.auto({
      ...
      store: ['validate', (results, callback) => {
        ...
        inbox.id = routes.inboxes + '/' + uuidV4();
  ...
  app.post(
    ...
    (req, res, next) => async.auto({
      ...
      store: ['compact', (results, callback) => {
        ...
        message.id = routes.messages + '/' + uuidV4();
  ...
  app.get(
    ...
    ensureAuthenticated,
    (req, res, next) => {
      const id = config.server.baseUri + '/' + req.originalUrl;
